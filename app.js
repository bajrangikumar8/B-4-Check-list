...
function render(){const total=data.length,done=data.filter(r=>roomStatus(r)==="Done").length,ip=data.filter(r=>roomStatus(r)==="In Progress").length,pe=data.filter(r=>roomStatus(r)==="Pending").length,overall=total?Math.round(data.reduce((s,r)=>s+pct(r),0)/Math.max(1,total)):0;$('stats').innerHTML=`<div class="stat"><b>${total}</b><span>Total Rooms</span></div><div class="stat"><b>${done}</b><span>Completed</span></div><div class="stat"><b>${ip}</b><span>In Progress</span></div><div class="stat"><b>${pe}</b><span>Pending</span></div><div class="stat wide"><b>${overall}%</b><span>Overall Completion</span></div>`;
$('floorCards').innerHTML=BUILDINGS.map(b=>b.parts.map(part=>{const a=data.filter(r=>r.building===b.id&&r.part===part.id),p=Math.round(a.reduce((s,r)=>s+pct(r),0)/Math.max(1,a.length));return `<div class="floorcard"><b>${b.name} • ${part.name}</b><div class="progress"><i style="width:${p}%"></i></div><small>${p}% • ${a.filter(r=>roomStatus(r)==="Done").length}/${a.length} done</small></div>`}).join('')).join('');
const q=$('search').value.toLowerCase(),b=$('buildingFilter').value,p=$('partFilter').value,f=$('floorFilter').value,st=$('statusFilter').value,list=data.filter(r=>(!q||r.id.toLowerCase().includes(q)||r.buildingName.toLowerCase().includes(q)||r.partName.toLowerCase().includes(q))&&(b==='ALL'||r.building===b)&&(p==='ALL'||`${r.building}:${r.part}`===p)&&(f==='ALL'||r.floor===f)&&(st==='ALL'||roomStatus(r)===st));$('roomCount').textContent=list.length+" rooms";$('rooms').innerHTML=list.map(r=>{const x=roomStatus(r),cl=x==="Done"?"done":x==="In Progress"?"ip":x==="Pending"?"pending":"ns";return `<div class="room" onclick="openRoom('${r.id}')"><div><b>${r.buildingName} • ${r.partName} • ${r.floor} • ${r.id.split('-').slice(-2).join('-')}</b><small>${pct(r)}%</small></div><span class="badge ${cl}">${x}</span></div>`}).join('')}
