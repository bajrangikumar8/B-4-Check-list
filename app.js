...
function roomStatus(r){const a=r.tasks.map(t=>t.status);if(a[9]==="Done")return "Handover";if(a.every(x=>x==="Done"||x==="N/A"))return "Done";if(a.includes("In Progress"))return "In Progress";if(a.includes("Pending"))return "Pending";if(a.slice(1).includes("Done"))return "In Progress";return "Not Started"}
function normalizeIncoming(x){
  ...
  const legacy=(incoming.length===104 && incoming.every(r=>r&&(!r.building||r.building==='B4'||!String(r.id).includes('B2-')&&!String(r.id).includes('B3-'))));
  const b4=fresh.filter(r=>r.building==='B4');
  const legacyById=new Map();
  if(legacy){incoming.forEach((r,i)=>{if(b4[i]) legacyById.set(b4[i].id,r);});}
  return fresh.map(f=>{
    const r=byId.get(f.id)||legacyById.get(f.id);
    return {...f,tasks:WORKS.map((w,j)=>({...f.tasks[j],...(r?.tasks?.[j]||{}),work:w}))};
  });
}

if(rawLocal) data=normalizeIncoming(rawLocal);
function saveLocal(){localStorage.setItem("roomsBuildingsV1",JSON.stringify(data))}
...
function render(){const total=data.length,done=data.filter(r=>roomStatus(r)==="Done"||roomStatus(r)==="Handover").length,ip=data.filter(r=>roomStatus(r)==="In Progress").length,pe=data.filter(r=>roomStatus(r)==="Pending").length,overall=total?Math.round(data.reduce((s,r)=>s+pct(r),0)/Math.max(1,total)):0;$('stats').innerHTML=`<div class="stat"><b>${total}</b><span>Total Rooms</span></div><div class="stat"><b>${done}</b><span>Completed</span></div><div class="stat"><b>${ip}</b><span>In Progress</span></div><div class="stat"><b>${pe}</b><span>Pending</span></div><div class="stat wide"><b>${overall}%</b><span>Overall Completion</span></div>`;
$('floorCards').innerHTML=BUILDINGS.map(b=>b.parts.map(part=>{const a=data.filter(r=>r.building===b.id&&r.part===part.id),p=Math.round(a.reduce((s,r)=>s+pct(r),0)/Math.max(1,a.length));return `<div class="floorcard"><b>${b.name} • ${part.name}</b><div class="progress"><i style="width:${p}%"></i></div><small>${p}% • ${a.filter(r=>roomStatus(r)==="Done"||roomStatus(r)==="Handover").length}/${a.length} done</small></div>`}).join('')).join('');
const q=$('search').value.toLowerCase(),b=$('buildingFilter').value,p=$('partFilter').value,f=$('floorFilter').value,st=$('statusFilter').value,list=data.filter(r=>(!q||r.id.toLowerCase().includes(q)||r.buildingName.toLowerCase().includes(q)||r.partName.toLowerCase().includes(q))&&(b==='ALL'||r.building===b)&&(p==='ALL'||`${r.building}:${r.part}`===p)&&(f==='ALL'||r.floor===f)&&(st==='ALL'||roomStatus(r)===st));$('roomCount').textContent=list.length+" rooms";$('rooms').innerHTML=list.map(r=>{const x=roomStatus(r),cl=x==="Done"||x==="Handover"?"done":x==="In Progress"?"ip":x==="Pending"?"pending":"ns";return `<div class="room" onclick="openRoom('${r.id}')"><div><b>${r.buildingName} • ${r.partName} • ${r.floor} • ${r.id.split('-').slice(-2).join('-')}</b><small>${pct(r)}%</small></div><span class="badge ${cl}">${x}</span></div>`}).join('')}
function refreshPartOptions(){const b=$('buildingFilter').value;let parts=[];if(b==='ALL'){parts=[{id:'B2:MAIN',name:'Building 2'},{id:'B3:A',name:'Building 3 • Part A'},{id:'B3:B',name:'Building 3 • Part B'},{id:'B3:C',name:'Building 3 • Part C'},{id:'B4:MAIN',name:'Building 4'}]}else{parts=BUILDINGS.find(x=>x.id===b).parts.map(x=>({...x,id:`${b}:${x.id}`}))}$('partFilter').innerHTML='<option value="ALL">All Parts</option>'+parts.map(x=>`<option value="${x.id}">${x.name}</option>`).join('');}
["search","floorFilter","statusFilter"].forEach(id=>$(id).addEventListener("input",render));
$("buildingFilter").addEventListener("change",()=>{refreshPartOptions();render()});
